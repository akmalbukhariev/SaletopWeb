...
function LoginForm() {
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
      <Stack spacing={3} sx={{ minWidth: '350px' }}>
       <Box>
          <TextField fullWidth label="User Name" variant="outlined" value={admin_id} onChange={(e)=> setAdminId(e.target.value)} required />
       </Box>
       <Box>
       ...
       <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <FormControlLabel
            ...
            label="Remember Me"
          />
         <Typography variant="body2" color="primary"><Link href="#" underline="hover"> Forgot Password?</Link></Typography>
       </Box>
        <Button type="submit" variant="contained" color="primary" disabled={loading}>{loading ? 'Logging in...' : 'Login'}</Button>
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1 }}>
          <Typography variant="body2" color="textSecondary">Don't have an account? <Link href="/auth/register" underline="hover">Sign Up</Link></Typography>
        </Box>
      </Stack>
    </Box>
  )
